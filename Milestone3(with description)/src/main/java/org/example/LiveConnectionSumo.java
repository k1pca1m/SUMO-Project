...
public class LiveConnectionSumo implements Runnable {
    ...
    // Using "volatile" for control flags because they are read/written by different threads (UI thread vs Sim thread).
    // 对控制标志使用 "volatile"，因为它们由不同的线程（UI 线程 vs 仿真线程）读/写。
    ...
    @Override public void run() {
        try {
            // Load native C++ libraries for SUMO.
            // 加载 SUMO 的原生 C++ 库。
            Simulation.preloadLibraries();

            // We construct a temporary routes file just for VTypes, because generic injection needs definitions.
            // 我们仅为 VTypes 构建一个临时路线文件，因为通用注入需要定义。
            String manualRou = buildRoutesFileWithVTypesOnly();

            // Build the command line arguments to start SUMO in GUI mode.
            // 构建命令行参数以在 GUI 模式下启动 SUMO。
            ...
            Simulation.start(cmd);// TraCI connection happens here / TraCI 连接发生在这里
            ...
            while (running) {
                if (!started) {
                    Thread.sleep(50);// Idle wait to reduce CPU usage when paused. / 空闲等待以减少暂停时的 CPU 使用率。
                    continue;
                }
                ...
                // Throttle simulation speed to be viewable by human eye.
                // 限制仿真速度，以便肉眼可以观察。
                Thread.sleep(sleepMs);

                // ---- vehicle metrics ----
                // DATA COLLECTION PHASE
                // Fetch all data in one go to minimize JNI/Socket crossings (expensive).
                // 数据收集阶段
                // 一次性获取所有数据，以尽量减少 JNI/Socket 交叉调用（开销大）。
                ...
                // Crucial: UI updates MUST happen on the Event Dispatch Thread (EDT).
                // We capture "final" variables to pass the snapshot of data to the UI.
                // 可视化更新
                // 关键：UI 更新必须发生在事件分发线程（EDT）上。
                // 我们捕获 "final" 变量，将数据快照传递给 UI。
            ...
            // Clean shutdown ensures SUMO process doesn't hang in background.
            // 干净的关闭确保 SUMO 进程不会在后台挂起。
