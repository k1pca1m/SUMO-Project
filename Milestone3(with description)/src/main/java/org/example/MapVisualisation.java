...
public final class MapVisualisation {
    ...
    /**
     * Parsing the .net.xml file to extract geometry (Shapes).
     * Why? SUMO provides coordinates, but drawing them requires converting
     * World Coordinates (Meters) to Screen Coordinates (Pixels).
     * 解析 .net.xml 文件以提取几何形状（Shapes）。
     * 为什么？SUMO 提供坐标，但绘制它们需要将世界坐标（米）转换为屏幕坐标（像素）。
     */
    private static java.util.List<RoadGeom> loadRoadGeometriesFromNet(File netFile) {
        ...
        // Storing as 'RoadGeom' allows efficient repainting in paintComponent without re-parsing.
        // 存储为 'RoadGeom' 允许在 paintComponent 中高效重绘，而无需重新解析。
        return out;
    }
    ...
    public static class MapPanel extends JPanel {
        ...
        // Transformation state: Zoom, Pan, Rotation.
        // 变换状态：缩放、平移、旋转。
        ...
        /**
         * The paint loop.
         * Swing uses double-buffering by default, so we just draw to the Graphics object.
         * 绘制循环。
         * Swing 默认使用双重缓冲，所以我们只需绘制到 Graphics 对象。
         */
        @Override protected void paintComponent(Graphics g) {
            super.paintComponent(g);
            Graphics2D g2 = (Graphics2D) g;

            // Anti-aliasing makes lines and vehicles look smoother (less jagged).
            // 抗锯齿使线条和车辆看起来更平滑（减少锯齿）。
            g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);

            Bounds b = getActiveBounds();

            // Draw order matters: Background -> Roads -> Markings -> Vehicles -> Overlay Labels.
            // 绘制顺序很重要：背景 -> 道路 -> 标线 -> 车辆 -> 覆盖标签。
            drawBackground(g2);
            drawRoads(g2, b);
            drawTlsMarkers(g2, b);

            // Draw Vehicles based on live data (ConcurrentHashMap ensures we don't crash if data updates while drawing).
            // 根据实时数据绘制车辆（ConcurrentHashMap 确保如果在绘制时数据更新，不会崩溃）。
            for (Map.Entry<String, Point2D.Double> e : vehicles.entrySet()) {
                ...
                // Check filter: if user unchecked "Show Cars", skip drawing.
                // 检查过滤器：如果用户取消选中 "显示汽车"，则跳过绘制。
                if (filter != null && !filter.allows(type, sp)) continue;

                // Convert World (x,y) to Screen (px,py).
                // 将世界坐标 (x,y) 转换为屏幕坐标 (px,py)。
                Point s = worldToScreen(p.x, p.y, b);
