...
public final class GUI {
    ...
    // ===================== FILTER STATE =====================
    // Filter class implements an interface defined in MapVisualisation.
    // This Decoupling allows the GUI to control what is drawn without the Map component needing to know about checkboxes.
    // Filter 类实现了 MapVisualisation 中定义的接口。
    // 这种解耦允许 GUI 控制绘制内容，而无需地图组件了解复选框的具体信息。
    public static class VehicleFilter implements MapVisualisation.Filter {
        public volatile boolean showCars = true;// volatile ensures thread visibility between UI thread and Paint thread.
        // volatile 确保 UI 线程和绘制线程之间的线程可见性。
    ...
    public static void launch() {
        // Set Look and Feel to System standard for a native experience.
        // 将外观设置为系统标准，以获得原生体验。
        ...
        // Why use a separate class (LiveConnectionSumo) for simulation logic?
        // Separation of Concerns: The GUI class handles "View", LiveConnectionSumo handles "Controller/Model".
        // It prevents the UI class from becoming a "God Class".
        // 为什么要使用单独的类 (LiveConnectionSumo) 进行仿真逻辑？
        // 关注点分离：GUI 类处理 "视图"，LiveConnectionSumo 处理 "控制器/模型"。
        // 这防止 UI 类变成一个臃肿的 "上帝类"。
        ...
        // ===== UI actions =====
        // Event Listeners: Linking UI actions to Logic.
        // Note: Long-running tasks (like starting simulation) are offloaded to threads inside 'live'.
        // 事件监听器：将 UI 操作链接到逻辑。
        // 注意：长时间运行的任务（如启动仿真）被卸载到 'live' 内部的线程中。
        ...
        // Exporting logic uses the current state of UI components to name files/headers.
        // 导出逻辑使用 UI 组件的当前状态来命名文件/标题。
        exportPdfBtn.addActionListener(e -> {
            // Render the chart component to an image buffer for the PDF.
            // 将图表组件渲染为图像缓冲区以用于 PDF。
            Object sel = routeCombo.getSelectedItem();
            String routeName = (sel instanceof VehicleInjection.RouteDef) ? ((VehicleInjection.RouteDef) sel).name : "";
