...
public class TrafficControl {
    ...
    // The simulation thread reads/writes TLS states, while the GUI thread (via buttons) sets manual overrides.
    // Thread-safety is required to prevent race conditions.
    // 为什么使用 ConcurrentHashMap？
    // 仿真线程读/写 TLS 状态，而 GUI 线程（通过按钮）设置手动覆盖。
    // 需要线程安全以防止竞争条件。
    private final ConcurrentHashMap<String, ManualTlsMode> manualTlsMode = new ConcurrentHashMap<>();

    // We store original programs to restore them when users click "Reset".
    // 我们存储原始程序，以便在用户点击 "重置" 时恢复它们。
    ...
    /**
     * Rule-Based Logic (Heuristic).
     * If cars are waiting, switch phases.
     * 基于规则的逻辑（启发式）。
     * 如果有车辆在等待，则切换相位。
     */
    private void applyRuleBasedTls(double simTime) {

        // If feature is disabled via GUI checkbox, exit immediately.
        // 如果该功能通过 GUI 复选框被禁用，立即退出。
        ...
        for (String tlsId : tlsIdsCached) {
            ...
            // "haltingVehiclesNearTls" queries lane data.
            // This allows the logic to be "Sensor-Actuator" based: Sense waiting cars -> Actuate Light.
            // "haltingVehiclesNearTls" 查询车道数据。
            // 这使得逻辑基于 "传感器-执行器"：感知等待的车辆 -> 操作信号灯。
            int demand = haltingVehiclesNearTls(tlsId);

            // 2. DECISION PHASE: State Machine / 决策阶段：状态机
            // Logic: If cars are waiting (demand > 0) AND we are in AUTO mode -> Switch to specialized phases.
            // 逻辑：如果有车在等待 (demand > 0) 并且我们处于自动模式 -> 切换到专用相位。
            ...
            // ACTUATOR PHASE: Change the light / 执行器阶段：改变信号灯
            if (rs.phase == RulePhase.AUTO) {

                // Trigger: Stop traffic to prepare for flush.
                // 触发：停止交通以准备放行。
                rs.phase = RulePhase.HOLD_RED;
                rs.untilSimTime = simTime + RULE_STOP_SEC;
                setTlsAll(tlsId, 'r');// Force RED / 强制红灯
                continue;
            }
