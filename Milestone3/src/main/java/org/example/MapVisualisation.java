...
public class MapVisualisation {
    ...
    public static class VehicleFilter {
        ...
        public boolean allowsType(String type) {
            if (LiveConnectionSumo.TYPE_CAR.equals(type)) return showCars;
            if (LiveConnectionSumo.TYPE_TRUCK.equals(type)) return showTrucks;
            if (LiveConnectionSumo.TYPE_BUS.equals(type)) return showBuses;
            return true;
        }
    ...
    public static class Bounds {
        ...
        public boolean sane() { return maxX > minX && maxY > minY; }
    }
    ...
    public static class RoadGeom {
        ...
        public RoadGeom(String edgeId, int laneIndex, boolean internal, float laneWidth, double[] xy) {
            this.edgeId = edgeId;
    ...
    public static volatile Bounds NET_BOUNDS = null;
    public static volatile Bounds FALLBACK_BOUNDS = new Bounds(-100, -100, 100, 100);

    public static final boolean DRAW_INTERNAL_EDGES = true;
    public static final boolean DRAW_ALL_LANES = true;
    public static final boolean DRAW_INTERNAL_CONNECTORS = DRAW_INTERNAL_EDGES;
    public static final boolean DRAW_LANE_MARKINGS = true;

    public static final double ROAD_THICKNESS_MULT = 1.25;
    public static final double ROAD_MIN_PX = 6.0;

    public static volatile List<RoadGeom> ROAD_GEOMS = Collections.emptyList();
    public static volatile Map<String, Point2D.Double> TLS_POSITIONS = Collections.emptyMap();
    public static volatile Map<String, String> TLS_LABELS = Collections.emptyMap();
    ...
    public static Bounds readConvBoundaryFromNet(File netFile) {
        try {
            ...
            Logging.LOG.log(Level.WARNING, "Failed reading convBoundary from net.xml", ex);
            return null;
        }
    }

    public static List<RoadGeom> loadRoadGeometriesFromNet(File netFile) {
        List<RoadGeom> out = new ArrayList<>();
        try {
            ...
            Logging.LOG.log(Level.WARNING, "Road geometry load failed; continuing without roads.", e);
        }
        return out;
    }

    public static Map<String, Point2D.Double> loadTlsPositionsFromNet(File netFile) {
        try {
            DocumentBuilderFactory dbf = DocumentBuilderFactory.newInstance();
            ...
            Logging.LOG.log(Level.WARNING, "Failed to load TLS positions from net.xml: " + netFile.getPath(), ex);
            return Collections.emptyMap();
        }
    }

    public static Map<String, String> buildTlsLabels(Iterable<String> tlsIds) {
    ...
    public static void initFromSumoCfg(String sumocfgPath) {
        try {
            String netPath = LiveConnectionSumo.readNetFileFromSumocfg(sumocfgPath);
            if (netPath == null || netPath.isBlank()) netPath = "final.net.xml";
            File netFile = LiveConnectionSumo.resolveRelativeToSumocfg(sumocfgPath, netPath);
            ...
            Logging.LOG.log(Level.WARNING, "Bounds init failed; using fallback.", e);
        }
    }
    ...
    public static class MapPanel extends JPanel {
        ...
        public MapPanel(VehicleFilter filter) {
            this.filter = filter;
            setOpaque(true);
            setFocusable(true);
        ...
        private void installMapInteraction() {
            MouseAdapter ma = new MouseAdapter() {
                ...
                @Override public void mouseDragged(MouseEvent e) {
                    ...
                    if (draggingPan) {
                        viewPanX += dx; viewPanY += dy; repaint();
                    } else if (draggingRotate) {
                        viewRotationRad += dx * 0.01; repaint();
                    }
                    lastMouse = e.getPoint();
                }
        ...
        private Point2D.Double baseWorldToScreen(double wx, double wy, Bounds b) {
            ...
            double screenY = panelH - sy; // flip y
        ...
        private void drawVehicle(Graphics2D g2, int sx, int sy, String type) {
            ...
            if (LiveConnectionSumo.TYPE_CAR.equals(type)) {
            ...
            } else if (LiveConnectionSumo.TYPE_TRUCK.equals(type)) {
        ...
        @Override protected void paintComponent(Graphics g) {
            ...
            for (Map.Entry<String, Point2D.Double> e : vehicles.entrySet()) {
                ...
                String type = vehicleTypes.getOrDefault(id, LiveConnectionSumo.TYPE_CAR);
                double sp = vehicleSpeeds.getOrDefault(id, 0.0);
                if (!filter.allows(type, sp)) continue;
    ...
    public static class TrendChartPanel extends JPanel {
        private final List<Double> avgWaitSec = new ArrayList<>();
        private final List<Double> throughputVph = new ArrayList<>();
        private final List<Double> congestion = new ArrayList<>();
        ...
        private void push(List<Double> series, double v) {
            series.add(v);
        ...
        private void drawSeries(Graphics2D g2, List<Double> series,
