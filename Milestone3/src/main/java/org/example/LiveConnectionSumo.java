...
public class LiveConnectionSumo implements Runnable {
    ...
    public final String SUMOCFG_PATH;
    ...
    public static void validateProjectSetup(String sumocfgPath) throws Milestone3Exception {
        File cfg = new File(sumocfgPath);
        if (!cfg.exists()) {
            throw new Milestone3Exception("Missing SUMO config file: '" + sumocfgPath + "'. " +
    ...
    // ===== sumocfg parsing utilities =====
    public static String readNetFileFromSumocfg(String sumocfgPath) {
        try {
            ...
            Logging.LOG.log(Level.WARNING, "Failed reading net-file from sumocfg", ex);
        }
        return null;
    }

    public static String readRouteFilesFromSumocfg(String sumocfgPath) {
        try {
            File f = new File(sumocfgPath);
            if (!f.exists()) return null;
            ...
            Logging.LOG.log(Level.WARNING, "Failed reading route-files from sumocfg", ex);
        }
        return null;
    }

    public static File resolveRelativeToSumocfg(String sumocfgPath, String pathMaybeRelative) {
    ...
    public static class MetricRow {
        public final String exportLocalTime;
        public final double simTime;
        public final int activeVehicles;
        public final int stoppedVehicles;
        public final double congestionIndex;
        public final double avgWaitSeconds;
        public final double meanSpeedMps;
        public final double throughputVph;
        public final int speedFactorUi;

        public final int visibleVehicles;
        public final int visibleCars;
        public final int visibleTrucks;
        public final int visibleBuses;

        public MetricRow(String exportLocalTime, double simTime, int activeVehicles, int stoppedVehicles,
    ...
    private final MapVisualisation.VehicleFilter filter;

    private final JLabel activeVehiclesLabel, visibleVehiclesLabel, byTypeLabel;
    private final JLabel avgWaitLabel, congestionLabel, throughputLabel, meanSpeedLabel;
    ...
    private final VehicleInjection vehicleInjection = new VehicleInjection();
    private final TrafficControl trafficControl = new TrafficControl();
    ...
    public LiveConnectionSumo(
            ...
            MapVisualisation.VehicleFilter filter,
            JLabel activeVehiclesLabel, JLabel visibleVehiclesLabel, JLabel byTypeLabel,
            JLabel avgWaitLabel, JLabel congestionLabel, JLabel throughputLabel, JLabel meanSpeedLabel,
            JLabel tlStateLabel,
            JComboBox<VehicleInjection.RouteDef> routeCombo,
            JComboBox<TrafficControl.TlsItem> tlCombo,
            Runnable onStopped
    ) {
        this.SUMOCFG_PATH = sumocfgPath;
        this.owner = owner;
    ...
    public VehicleInjection getVehicleInjection() { return vehicleInjection; }
    public TrafficControl getTrafficControl() { return trafficControl; }
    ...
    private double computeAvgWaitSeconds(StringVector vIds) {
        ...
        for (int i = 0; i < vIds.size(); i++) {
            ...
            if (!Double.isNaN(w) && !Double.isInfinite(w)) { sum += w; cnt++; }
        }
        if (cnt == 0) return -1.0;
        return sum / cnt;
    }
    ...
    private double computeMeanSpeed(StringVector vIds) {
        ...
        double sum = 0.0; int cnt = 0;
        for (int i = 0; i < vIds.size(); i++) {
            try { sum += Vehicle.getSpeed(vIds.get(i)); cnt++; } catch (Exception ignore) {}
        }
    ...
    private int countStopped(StringVector vIds) {
        int stopped = 0;
        for (int i = 0; i < vIds.size(); i++) {
            try { if (Vehicle.getSpeed(vIds.get(i)) < 0.1) stopped++; } catch (Exception ignore) {}
        }
        return stopped;
    }

    private int recordArrivals(double simTime, Set<String> currentIdsSet) {
        try {
    ...
    private void trimThroughputWindow(double simTime) {
        while (!arrivalTimes.isEmpty()) {
            ...
            if ((simTime - t) > THROUGHPUT_WINDOW_SEC) arrivalTimes.removeFirst();
            else break;
        }
    }

    private double computeThroughputVph() {
        double windowHours = THROUGHPUT_WINDOW_SEC / 3600.0;
    ...
    public void exportMetricsCsv(Component parent, String selectedRouteName) {
        ...
        try (PrintWriter pw = new PrintWriter(new OutputStreamWriter(new FileOutputStream(file), "UTF-8"))) {
            ...
            Logging.LOG.log(Level.SEVERE, "Export CSV failed", ex);
    ...
    public void exportSummaryPdf(JFrame parent, String selectedRouteName, BufferedImage chartImage) {
        ...
        lines.add("Selected route: " + (selectedRouteName == null ? "" : selectedRouteName));
        lines.add("Filters: cars=" + filter.showCars + ", trucks=" + filter.showTrucks +
                ", buses=" + filter.showBuses + ", minSpeed(m/s)=" + String.format(Locale.US, "%.2f", filter.minSpeedMps));
        ...
        try {
            SimplePdfWriter.writeOnePageReport(file, "Traffic Simulation - Summary Report", lines, chartImage);
        } catch (Exception ex) {
            Logging.LOG.log(Level.SEVERE, "Export PDF failed", ex);
    ...
    @Override public void run() {
        try {
            ...
            // init routes + map from files before SUMO starts
            vehicleInjection.loadTripRoutesFromRou(SUMOCFG_PATH);
            MapVisualisation.initFromSumoCfg(SUMOCFG_PATH);

            String manualRou = vehicleInjection.buildRoutesFileWithVTypesOnly();

            StringVector cmd = new StringVector();
            cmd.add("sumo-gui");
            cmd.add("-c"); cmd.add(SUMOCFG_PATH);
            ...
            trafficControl.rebuildTrafficLightDropdown(tlCombo, tlStateLabel);
            ...
            while (running) {
                ...
                trafficControl.applyManualOverrideIfNeeded();
                ...
                recordArrivals(simTime, currentIdSet);
                ...
                for (int i = 0; i < vIds.size(); i++) {
                    ...
                    String type;
                    if (id.startsWith("car_")) type = TYPE_CAR;
                    else if (id.startsWith("truck_")) type = TYPE_TRUCK;
                    else if (id.startsWith("bus_")) type = TYPE_BUS;
                    else type = TYPE_CAR;
                    ...
                    if (filter.allows(type, sp)) {
                        visibleCount++;
                        if (TYPE_CAR.equals(type)) visCar++;
                        else if (TYPE_TRUCK.equals(type)) visTruck++;
                        else if (TYPE_BUS.equals(type)) visBus++;
                    }
                }
                ...
                if (lastLoggedSimTime < 0 || (simTime - lastLoggedSimTime) >= LOG_EVERY_SIM_SECONDS) {
                    ...
                    trendChart.addSample(avgWaitSec < 0 ? 0.0 : avgWaitSec, throughputVph, congestion);
                }

                String tlsStateStr = trafficControl.buildTlsStateLine();

                final Map<String, Point2D.Double> positionsF = positions;
                ...
                final String tlsStateF = tlsStateStr;
                ...
                SwingUtilities.invokeLater(() -> {
                    ...
                    throughputLabel.setText(String.format(Locale.US,
                            "Throughput: %.1f v/h (last %.0f s)", throughputVphF, THROUGHPUT_WINDOW_SEC));
            ...
            Logging.LOG.log(Level.SEVERE, "Simulation thread crashed", ex);
            if (onStopped != null) SwingUtilities.invokeLater(onStopped);
        }
    }

    // ===== tiny PDF writer (same as yours, moved here) =====
